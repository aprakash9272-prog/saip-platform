...
- **Sprint 10 — Security Overlap & Optimization Engine.** Built on top of the Coverage, Gap, and Recommendation Engines: `OverlapEngine` detects duplicate capabilities (flagging cross-vendor duplicates specifically), product/module/framework overlap, redundant licenses, and unused (purchased-but-never-enabled) capabilities across deployed products. Calculates an overlap %, an optimization score, a vendor consolidation score, a license reduction opportunity, a cost optimization score, and an operational complexity score — all deterministic, rule-based, and cross-referencing the Recommendation Engine's product comparison. Exposed via `/analysis/overlap*` APIs with JSON/Excel/PDF export, and a dedicated Overlap page (executive summary, vendor comparison, overlap matrix, capability heatmap, duplicate products, optimization opportunities, filterable/sortable/searchable duplicate-capability table).
- **Sprint 11 — Scenario Simulation Engine.** Closes out the deterministic analysis suite: `SimulationEngine` lets an architect simulate 12 scenario types (add/remove/replace product, upgrade/downgrade edition, change licensing tier, enable/disable module, change deployment model, change availability status, consolidate vendors, remove duplicate products) against an assessment's real Deployed product assignments, without ever modifying them. It reuses the Coverage, Gap, Recommendation, and Overlap Engines completely unmodified for both a "current" and "proposed" calculation pass; the hypothetical mutation is staged with `session.flush()` (never `session.commit()`) so the engines see it, then unconditionally rolled back before the request returns — verified against both SQLite and PostgreSQL that a flush-then-rollback can never leak into the durable database, unlike the existing service layer's committing CRUD methods. Produces ten current-vs-proposed metric deltas (coverage, gap, overlap, recommendation, risk, cost, complexity, vendor count, license count, framework coverage) each classified Improvement/Regression/Neutral, capability/vendor/framework comparison tables, and a deterministic templated executive summary — no AI, no LLM. Exposed via `/analysis/simulation*` APIs (`POST` to run and persist a report, `GET .../{id}` to retrieve one later, `.../summary` and `.../export` for JSON/Excel/PDF), and a dedicated Simulation page (scenario builder wizard, current-vs-proposed metric cards, before/after charts, coverage heatmaps, vendor/framework/capability comparison tables).

The AI Assistant (part of Sections 9–13) is the only piece of the original blueprint intentionally not yet implemented — Sprints 3–11 exist to build the knowledge base, customer workspace, and the full deterministic coverage/gap/recommendation/overlap/simulation calculation suite; AI-assisted reasoning remains a future sprint.
