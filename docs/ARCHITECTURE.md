...
Route registration order matters where static and dynamic paths share a prefix: within `api/routes/analysis.py`, each analysis resource's `export`/`summary` (and, for gaps, `domains`) sub-routes are registered *before* its dynamic `/{assessment_id}` route — `/analysis/gaps/*`, `/analysis/recommendations/*`, and `/analysis/overlap/*` all follow this pattern, since FastAPI matches path operations in registration order and a dynamic segment would otherwise swallow the static ones.
...
- `simulation_engine.py`, `cost_engine.py` — placeholders (`raise NotImplementedError`) for future sprints.
...
src/components/assessments/    Assessment project page, Product Assignment wizard, the Coverage
                                ...
                                and the Overlap page (vendor comparison, overlap matrix, capability
                                heatmap, duplicate products, optimization opportunities)
...
Flat catalog entities (vendors, products, customers, ...) go through the generic config-driven `ResourcePage`. Entities with nested detail views, cross-entity dashboards, cascading selection (vendor → product → edition → module), file import/export, or computed reports (Customer, Assessment Project, Product Assignment, Capabilities, Product Mappings, Coverage Analysis, Gap Analysis, Recommendations, Overlap) use bespoke page components that compose the same shared UI primitives (`Dialog`, `Table`, `Card`, shadcn/ui) directly, plus `recharts` for charts. The Gap Analysis, Recommendations, and Overlap pages (`/assessments/{id}/gaps`, `/recommendations`, `/overlap`) are standalone routes rather than embedded sections — search/filter/sort state for a full table warranted its own page instead of crowding the Assessment Project page further.
